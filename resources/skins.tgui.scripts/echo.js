...
function init() {
  ...
  const alertIcon = notifications.querySelector('#pt-notifications-alert > a > .tgui-icon').cloneNode();
  const noticeIcon = notifications.querySelector('#pt-notifications-notice > a > .tgui-icon').cloneNode();
  ...
  const setupFosterHome = (badge, anchor) => {
    ...
    anchor.classList.add('tgui-header__button', 'tgui-echo-notification-badge');
    ...
    for (const mutation of mutations) {
      if (mutation.type === 'childList') {
        ...
        for (const removedNode of removedNodes) {
          if (!(removedNode instanceof HTMLSpanElement) || !removedNode.classList.contains('tgui-icon')) {
            return;
          }
  ...
  initObserver.observe(notifications, {
    childList: true,
    subtree: true,
  });
}
