...
/**
 * TGUIComponentPageHeading component
 */
class TGUIComponentPageHeading implements TGUIComponent {
	...
	private function buildUserTagline(): string {
		...
		if ( $user ) {
			...
			if ( is_string( $regDate ) ) {
				...
				$msgRegDate = $localizer->msg( 'tgui-tagline-user-regdate', $this->pageLang->userDate( new MWTimestamp( $regDate ), $this->user ), $user );
	...
	private function determineTagline(): string {
		...
		if ( empty( $namespaceText ) ) {
			if ( !$localizer->msg( 'tgui-tagline' )->isDisabled() ) {
				return $localizer->msg( 'tgui-tagline' )->parse();
			} else {
				return $localizer->msg( 'tagline' )->parse();
			}
		}

		$msg = $localizer->msg( 'tgui-tagline-ns-' . strtolower( $namespaceText ) );
		...
		if ( $title->isTalkPage() ) {
			...
			return $localizer->msg( 'tgui-tagline-ns-talk' )->parse();
		}
